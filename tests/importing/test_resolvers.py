...
class TestBuiltinResolver:
    ...
    @pytest.fixture
    def resolver(self, security):
        return BuiltinResolver(security)
    ...
    def test_resolve_builtin(self, resolver):
        ...
        assert result.metadata['type'] == 'builtin'
    ...
    def test_builtin_source_validation(self):
        # Must specify name
        with pytest.raises(ImportingError, match="name must be specified"):
            ImportSource(SourceType.BUILTIN)

        # Location should not be specified
        with pytest.raises(ImportingError, match="location should not be specified"):

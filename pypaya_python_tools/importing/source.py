...
class SourceType(Enum):
    ...
    MODULE = auto()    # Python module
    FILE = auto()      # Python file
    BUILTIN = auto()   # Built-in module/object
...
class ImportSource:
    ...
    def __post_init__(self):
        ...
        if self.type == SourceType.BUILTIN:
            if not self.name:
                raise ImportingError("name must be specified for BUILTIN source")
            if self.location:
                raise ImportingError("location should not be specified for BUILTIN source")
        else:
            if not self.location:
                raise ImportingError(f"location must be specified for {self.type.name} source")
    ...
    def is_builtin(self) -> bool:
        return self.type == SourceType.BUILTIN

...
def import_object(
    ...
    Args:
        path_spec: Object location specified as:
            ...
            - file:class format ('/path/to/module.py:MyClass')
    ...
    Examples:
        ...
        # File:class format
        obj5 = import_object('/path/to/module.py:MyClass')
    """
    manager = ImportManager(security)
    ...
    # Handle file:class format
    if (name is None and ':' in path_str and
            (path_str.endswith('.py') or '/' in path_str or '\\' in path_str)):
        # Split at the last colon to handle paths with multiple colons
        file_path, class_name = path_str.rsplit(':', 1)
        path_spec = file_path
        name = class_name

    # Handle full dotted path format
    elif (name is None and '.' in path_str and
